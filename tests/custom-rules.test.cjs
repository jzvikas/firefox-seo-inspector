...
test('normalization clamps numeric values and swaps inverted text ranges safely', () => {
  const config = CustomRules.normalize({
    thresholds: {
      titleMin: 90,
      titleMax: 30,
      descriptionMin: 300,
      descriptionMax: 100,
      oversizedImageRatio: 999,
      imageMaxBytes: -1,
    },
  });
...
test('validation reports invalid numeric values and inverted ranges instead of silently saving them', () => {
  const errors = CustomRules.validate({
    thresholds: {
      titleMin: 70,
      titleMax: 60,
      descriptionMin: 50,
...
test('severity overrides accept only supported severities and stable rule identifiers', () => {
  const config = CustomRules.normalize({
    severityOverrides: {
      'title.short': 'critical',
      'schema.invalid': 'INFO',
      'title.long': 'fatal',
      'bad id!': 'warning',
    },
  });
  assert.deepEqual(config.severityOverrides, {
    'schema.invalid': 'info',
    'title.short': 'critical',
  });
});
...
test('SEO core options contain only normalized audit policy fields', () => {
  const options = CustomRules.toSeoCoreOptions({
    thresholds: {
      titleMin: 10,
      titleMax: 70,
      descriptionMin: 40,
      descriptionMax: 180,
      oversizedImageRatio: 3,
      imageMaxBytes: 100000,
    },
    required: { schema: true, https: true },
    disabledChecks: ['links.javascript'],
    severityOverrides: { 'title.short': 'critical' },
  });
  assert.equal(options.titleMin, 10);
  assert.equal(options.titleMax, 70);
  assert.equal(options.oversizedImageRatio, 3);
  assert.equal(options.required.schema, true);
  assert.equal(options.required.https, true);
  assert.deepEqual(options.disabledChecks, ['links.javascript']);
  assert.equal(options.severityOverrides['title.short'], 'critical');
  assert.equal(Object.prototype.hasOwnProperty.call(options, 'imageMaxBytes'), false);
});
...
test('image file-size issue is emitted only for known sizes above the configured threshold', () => {
  ...
  const config = CustomRules.normalize({
    thresholds: { imageMaxBytes: 100000 },
    severityOverrides: { 'images.fileSize': 'critical' },
  });
...
test('image file-size check can be disabled independently', () => {
  const issue = CustomRules.imageSizeIssue({ rows: [{ sizeBytes: 999999, image: {} }] }, {
    thresholds: { imageMaxBytes: 100000 },
    disabledChecks: ['images.fileSize'],
  });
