...
test('URL comparison resolves custom rules and expectations for each target hostname', async () => {
  ...
  vm.runInContext(source('src/lib/product-page-audit.js'), context, { filename: 'product-page-audit.js' });
  vm.runInContext(source('src/lib/category-page-audit.js'), context, { filename: 'category-page-audit.js' });
  ...
  assert.equal(profiled.productAudit.applicable, false);
  assert.equal(profiled.categoryAudit.applicable, false);
  ...
  assert.equal(globalOnly.productAudit.applicable, false);
  assert.equal(globalOnly.categoryAudit.applicable, false);
